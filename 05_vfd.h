...
private:
    ...
    //pinLed.on();
		if (pinBusy.isActive()) {
			...
			while (pinBusy.isActive())
				;  // wait for VFD to become ready
        // FIXME This risks hanging the entire system, we need some form of abort switch
			logger.logln("ok");
		}
		Serial1.print(text.c_str());
		currentCursorPosition += text.length(); // TODO This assumes no non-printing characters
    //logger.log("Current pos: "); logger.logln(currentCursorPosition);
    //pinLed.off();
    //logger.end();
	}
...
public:
    ...
    //logger.log("Serial1.availableForWrite()="); logger.logln(Serial1.availableForWrite());
		...
		if (numSlots) {
			...
			for (int slot=0; slot<numSlots; slot++) {
				int centre = 1 + slot*numCharsPerSlot + numCharsPerSlot/2;
  ...
  **/
	...
	**/
    ...
    //logger.log("Need to adjust by "); logger.log(adjustment); logger.logln(" positions");
    ...
    //logger.begin("moveTo");
      ...
      //logger.log("Current pos: "); logger.logln(currentCursorPosition);
      //logger.log("Desired pos: "); logger.logln(desiredPos);
			int adjustment = desiredPos-currentCursorPosition;
      //logger.log("Need to move "); logger.log(adjustment); logger.logln(" positions");
			if (adjustment) {
				moveBy(adjustment);
			}
		} else {
...
//    cursorBlink(); // For debug
	...
	**/
    ...
    //logger.log("col: ");
    //logger.logln(col);
    //logger.log("pos: "); logger.log(row); logger.log(", "); logger.log(col);
    //logger.log("text: ");
